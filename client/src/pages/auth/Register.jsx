...
function Register() {
  ...
  const onFinish = async (values) => {
    setLoading(true);
    try {
      var res = await fetch(import.meta.env.VITE_APP_SERVER_URL +"/api/auths/register", {
        method: "POST",
        body: JSON.stringify(values),
        headers: { "Content-type": "application/json; charset=UTF-8" },
      });
      if (res.status === 200) {
        message.success("Üyelik işlemi başarılı");
        navigate("/login");
        setLoading(false);
      }
    } catch (error) {
              ...
              rules={[
                ...
                ({ getFieldValue }) => ({
                  validator(_, value) {
                    ...
                    return Promise.reject(new Error("Parola Eşleşmiyor!"));
                  },
                }),
              ]}
            >
